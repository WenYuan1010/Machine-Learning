...
import time
from tqdm import *
import win32api,win32con#弹窗提醒库
...
df_companys = pd.read_excel(r'C:\Users\WenYuan\Desktop\{}.xlsx'.format(inputname))
companys = df_companys['企业简称'].tolist()
...
try:
    ...
    for company in tqdm(companys):
        headers = {
            ...
            'cookie':'qcc_did=07894e4f-cda0-4e8b-a176-cc1fb53bfbf7; UM_distinctid=17f8811b31ecb6-0f6211d9c81289-56171d51-144000-17f8811b31fc64; _uab_collina=165000968522032972050308; QCCSESSID=e10a03a4b4f8e48890d426207d; acw_tc=6fa1039f16564005406334980e7384b506ad91b2077e4af1310670e67e; CNZZDATA1254842228=1295480780-1647246194-https%253A%252F%252Fwww.baidu.com%252F%7C1656398020'
        ...
        if len(soup.find_all('a', {'class': 'title copy-value'})) == 0:
            ...
            if len(soup.find_all('a', {'class': 'title copy-value'})) == 0:
                ...
                if len(soup.find_all('a', {'class': 'title copy-value'})) == 0:
                    ...
                    a.append([company, company_list, management_forms])
                    print([company, company_list, management_forms])
            elif len(list(soup.find_all('a', {'class': 'title copy-value'})[0].next_siblings)) == 1:
                company_list = soup.find_all('a', {'class': 'title copy-value'})[0].get_text().strip()
                a.append([company, company_list, ''])
                print([company, company_list, ''])
            ...
            company_list = soup.find_all('a', {'class': 'title copy-value'})[0].get_text().strip()
            management_forms = soup.find_all('a', {'class': 'title copy-value'})[0].next_sibling.next_sibling.get_text().strip()
            a.append([company, company_list, management_forms])
            print([company, company_list, management_forms])
